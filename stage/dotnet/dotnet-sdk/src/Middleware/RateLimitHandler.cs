...
using GitHub.Octokit.SDK.Middleware;
...
/// <param name="options">Optional: The options to use for the handler.</param>
public class RateLimitHandler(
    ILogger<RateLimitHandler> logger,
    IRateLimitHandlerOptions? options = null) : DelegatingHandler
{
    /// <summary>
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        if (rateLimit is not RateLimitType.None)
        {
            ...
            if (rateLimit is RateLimitType.Primary)
            {
                ...
                logger.LogPrimaryRateLimitExceeded(
                    response.Headers.GetValues(XRateLimitResetKey).FirstOrDefault(), retryAfterDuration?.TotalSeconds ?? 0);

                logger.LogRateLimitInformation(
                    XRateLimitLimitKey,
                    response.Headers.GetValues(XRateLimitLimitKey).FirstOrDefault(),
                    XRateLimitUsedKey,
                    response.Headers.GetValues(XRateLimitUsedKey).FirstOrDefault(),
                    XRateLimitResourceKey,
                    response.Headers.GetValues(XRateLimitResourceKey).FirstOrDefault());

            }
            else if (rateLimit is RateLimitType.Secondary)
            {
                logger.LogSecondaryRateLimitExceeded(retryAfterDuration?.TotalSeconds ?? 0);
            }
            ...
            {
                logger.LogInvalidRateLimitResetTime(retryAfterDuration?.TotalSeconds ?? 0);
            }
    ...
    protected internal TimeSpan? ParseRateLimit(HttpResponseMessage response, DateTime utcNow)
    {
    ...
    /// <param name="utcNow">The current UTC time.</param>
    /// <returns>The duration to wait before retrying the request.</returns>
    protected internal TimeSpan? ParseRetryAfterHeader(HttpResponseMessage response, DateTime utcNow)
    {
    ...
    /// <param name="utcNow">The current UTC time.</param>
    /// <returns>The duration to wait before retrying the request.</returns>
    protected internal TimeSpan? ParseXRateLimitReset(HttpResponseMessage response, DateTime utcNow)
    {
